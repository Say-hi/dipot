<template>
  <Dropdown ref="dropdown" @on-click="handleClick" :class="hideTitle ? '' : 'collased-menu-dropdown'" :transfer="hideTitle" :placement="placement">
    ...
    <DropdownMenu ref="dropdownItem" slot="list">
...
export default {
  ...
  mounted () {
    ...
    if (this.parentItem && this.parentItem && this.parentItem.meta && this.parentItem.meta.maxWidth) {
      this.$refs.dropdownItem.$el.style.maxWidth = this.parentItem.meta.maxWidth
    }
  }
}
</script>
